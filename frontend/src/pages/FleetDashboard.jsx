...
const DirectionScreen = ({ headingLabel, dirLabel, dest, originLoc, destLoc, guest, onAction, actionLabel }) => (
  ...
  >
    ...
    <div className="px-5 pt-6 pb-3 flex-shrink-0">
      {headingLabel && (
        <p className="text-[9px] font-bold uppercase tracking-[0.28em] text-muted-foreground mb-1.5">
          {headingLabel}
        </p>
      )}
    ...
    {/* Map */}
    <div
      className="mx-5 flex-1 rounded-2xl overflow-hidden border border-border mb-3"
    ...
    {/* CTA */}
    <div
      className="px-5 py-4 pb-24 flex-shrink-0"
...
export default function FleetDashboard() {
  ...
  // 'idle' | 'routing-pickup' | 'routing-drop' | 'instant-select'

  const [queue, setQueue] = useState(fleetQueue);
  const [trip, setTrip] = useState(null); // { from, to, guest }

  // Instant pickup selection state
  const [pickupFrom, setPickupFrom] = useState('');
  const [pickupTo, setPickupTo] = useState('');
  const [ipStep, setIpStep] = useState(1); // 1=from, 2=to
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col overflow-hidden">
      ...
      <header
        className="sticky top-0 z-20 px-5 pt-12 pb-3 border-b border-border flex-shrink-0"
        style={{ background: 'hsl(0 0% 4% / 0.97)', backdropFilter: 'blur(16px)' }}
      >
        <div className="flex bg-secondary rounded-pill p-1">
          <button
            className="flex-1 py-2.5 text-center text-[11px] font-medium text-muted-foreground tracking-[0.22em] uppercase"
            style={{ transition: 'color 0.2s' }}
            onClick={() => navigate('/valet')}
          >
            Valet
          </button>
          <div className="flex-1 py-2.5 rounded-pill bg-foreground text-center text-[11px] font-bold text-background tracking-[0.22em] uppercase">
            Fleet
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 relative overflow-hidden">
        <AnimatePresence mode="wait">
              ...
              <div className="px-5 pt-4 pb-3">
                <div
                  ...
                  style={{
                    ...
                    border: '1px solid hsl(142 70% 42% / 0.22)',
                  }}
                >
          ...
          {step === 'instant-select' && (
            ...
            >
              <div className="px-5 pt-6 pb-4 flex-shrink-0">
                ...
                {ipStep === 2 && (
                  <p className="text-sm text-muted-foreground mt-0.5">From: {pickupFrom}</p>
                )}
              </div>
              ...
              {ipStep === 2 && pickupTo && (
                ...
                >
                  <div
                    className="flex items-center gap-3 bg-secondary rounded-xl px-4 py-3 mb-4"
                  >
          ...
          {step === 'routing-pickup' && trip && (
            <DirectionScreen
              key="routing-pickup"
              headingLabel="Active Trip · Step 1 of 2"
          ...
          {step === 'routing-drop' && trip && (
            <DirectionScreen
              key="routing-drop"
              headingLabel="Active Trip · Step 2 of 2"
              dirLabel="Drive to"
              dest={trip.to}
              originLoc={trip.from}
              destLoc={trip.to}
